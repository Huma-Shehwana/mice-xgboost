...
sampler_xgb <- function(data, m, ignore, where, imp, blocks, method,
  ...
  train_error <-  vector("list", ncol(data))
        ...
        for (h in visitSequence) {
          ...
          if (univ) {
            for (j in b) {
              ...
              imp[[j]][, i] <-tmp$imputes
              train_error[[j]][1,i] <-tmp$err #each column is 
  ...
  list(iteration = maxit, imp = imp, chainMean = chainMean, chainVar = chainVar, trainError=train_error)
}


sampler.univ.xgb <- function(data, r, where, pred, formula, method, yname, k,
